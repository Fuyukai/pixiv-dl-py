...
class Filterer(object):
    ...
    def symlink_filtered(self, output_dir: Path, *,
                         suppress_filter_messages: bool = False):
        """
...
def main():
    ...
    # output controls
    parser.add_argument(
        "-d", "--db", help="The db dir (containing config, etc)",
        default="output/"
    )
    parser.add_argument(
        "-s", "--subdir",
    ...
    parser.add_argument(
        "-o", "--output", help="The directory to output the filtered items",
        default="filtered/"
    )
    parser.add_argument(
        "--suppress-extra", help="Suppress unfiltered entry message printing",
        action="store_true", default=False,
    )

    # filter controls
    parser.add_argument("--filter-field", help="Adds a filter on a field",
                        action="append", default=[])
    parser.add_argument("--filter-tag", help="Adds a filter on a tag",
                        action="append", default=[])

    r18_group = parser.add_mutually_exclusive_group()
    r18_group.add_argument("--filter-r18", help="Adds a filter for all R-18 content",
                           action="store_true")
    r18_group.add_argument("--filter-not-r18", help="Adds a filter for all non R-18 content",
                           action="store_true")  # for the ultimate in porn downloading

    parser.add_argument("--filter-user", help="Adds a filter on a user",
